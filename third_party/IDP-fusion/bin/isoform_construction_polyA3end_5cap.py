...
LR_gpd_dt = {}
candidate_dt = {}
fusion_isoform_2_readname_dict = {}
LR_gpd = open(nonredundant_LR_gpd_filename ,'r')
...
for line in LR_gpd:
    ...
    name_ls = ls[0].split('|')
    if (name_ls[1][0] == "N"):
        fusion_dir = '*'   # Normal candidate
    else:
        if ('+' in name_ls[1]):
            fusion_dir = '+'
        elif ('-' in name_ls[1]):
            fusion_dir = '-'
        fusion_endpoint = int(re.split(r'\+|\-', (name_ls[1]))[1])

    # In case of fusion genes, we allow single exons
    if (fusion_dir == '*'):
        first_jun = [int(exon_end_list[0]), int(exon_start_list[1]) ]
        last_jun = [int(exon_end_list[-2]), int(exon_start_list[-1]) ]
    else:
        if(len(exon_start_list) == 1):
            jun_point = (int(exon_start_list[0]) + int(exon_end_list[0])) / 2
            # fake splice junction
            first_jun = [jun_point, jun_point + 3]
            last_jun = [jun_point, jun_point + 3]
        else:
            first_jun = [int(exon_end_list[0]), int(exon_start_list[1]) ]
            last_jun = [int(exon_end_list[-2]), int(exon_start_list[-1]) ]
...
#    LR_gpd_dt[chr_name][exon_start_list[0]][exon_end_list[-1]] = jun_lk
    ...
    # Update strand for single-exon fusion isoforms
    if ((fusion_dir != "*") and (len(exon_start_list) == 1)):
    
        gpd_strand_ls =['']
        #print ">>>>strand: " + line
        #print start_jun_ls + end_jun_ls
        for jun_ls in (start_jun_ls + end_jun_ls):
            jun_locus = chr_name + ':' + str(jun_ls[0]) + '-' +str(jun_ls[1])
            if jun_strand_dt.has_key(jun_locus):
                gpd_strand_ls.append( jun_strand_dt[jun_locus] )
        ...
        if L_gpd_strand_s == 1:
            ...
            # Choose the most occurred one
            #if (gpd_strand_ls.count('+') > gpd_strand_ls.count('-')):
            #    gpd_strand = '+'
            #else:
            #    gpd_strand = '-'
            gpd_strand = ""
            print "potential gene fusion:", line.strip()
...
#    junlk_set = set()
    ...
    if I_3end == '+':
        ...
        if (fusion_dir == '-'):  # Note: this is not expected
            ending_boundary = fusion_endpoint
            print "Warning: This condition(0) is not expected: " + line
        
        if (fusion_dir == '+'):
            left_ending_ls, start_jun_lk_ls = make_startjun_threeend_combination(ref_start_lk_ls, start_jun_ls,[fusion_endpoint],exon_len_limit,exon_boundary_ls, exon_start_list[0])
        elif not fwd_5end_dt.has_key(locus):
        ...
        for start_junlk_str in start_junlk_str_ls:
            ...
            if (len(item) > 0):
                ...
                twoending_str =  fiveend_ending_str + '_' + str(ending_boundary) 
                candidate_dt[locus][item][gpd_strand].add( twoending_str )
                if (fusion_dir != '*'):
                    print_str = 'kinfai'.join([locus, item, gpd_strand, twoending_str])
                    if (not fusion_isoform_2_readname_dict.has_key(print_str)):
                        fusion_isoform_2_readname_dict[print_str] = set()
                    fusion_isoform_2_readname_dict[print_str].add(ls[0])
            else:
                # Special case for handling fusion single exons
                
                if (fiveend_ending_str != -1 or ending_boundary != -1):
                    if not candidate_dt[locus].has_key(item):
                        candidate_dt[locus][item] = {}
                    if not candidate_dt[locus][item].has_key(gpd_strand):
                        candidate_dt[locus][item][gpd_strand] = set()

                    # PT: or we can ignore this case (200 is the same number used
                    #     in merge_isoform_....py
                    if (fiveend_ending_str == -1):
                        print "Unexpected: fiveend_ending_str == -1 :" + line
                        exit(1)
                    temp_end = int( item.split('_')[-1] )
                    if (ending_boundary - temp_end > exon_len_limit):
                        ending_boundary = fiveend_ending_str + 200
                            
                    twoending_str = str(fiveend_ending_str) + '_' + str(ending_boundary) 
                    candidate_dt[locus][item][gpd_strand].add( twoending_str )
                    if (fusion_dir != '*'):
                        print_str = 'kinfai'.join([locus, item, gpd_strand, twoending_str])
                        if (not fusion_isoform_2_readname_dict.has_key(print_str)):
                ...
                if (fiveend_ending_str == -1 and ending_boundary == -1):
                    print "Unexpected: both -1:" + line
                    exit(1)
...
#            junlk_set.add( start_junlk_str + '-' + jun_lk )
    ...
    if I_3end == '-':
        ...
        if ( fusion_dir == '+'):
            ending_boundary = fusion_endpoint
            print "Warning: This condition(1) is not expected: " + line

        if ( fusion_dir == '-'):
            right_ending_ls, end_jun_lk_ls = make_endjun_threeend_combination([[last_jun]], end_jun_ls,[fusion_endpoint],exon_len_limit,exon_boundary_ls, exon_end_list[-1])
        elif not rev_5end_dt.has_key(locus):
        ...
        for end_junlk_str in end_junlk_str_ls:
            ...
            if (len(item) > 0):
                ...
                twoending_str = str(ending_boundary) + '_' + fiveend_ending_str 
                candidate_dt[locus][item][gpd_strand].add( twoending_str )
                if (fusion_dir != '*'):
                    print_str = 'kinfai'.join([locus, item, gpd_strand, twoending_str])
                    if (not fusion_isoform_2_readname_dict.has_key(print_str)):
...
#                    print "KKKK",left_ending_ls, start_jun_lk_ls , right_ending_ls, end_jun_lk_ls 
                ...
                end_jun_lk_ls = make_endjun_combination([[last_jun]],end_jun_ls,exon_len_limit)
                if (fusion_dir == '+'):
                    left_ending_ls = [fusion_endpoint]*len(start_jun_lk_ls)
                else:
                    left_ending_ls = [-1]*len(start_jun_lk_ls)
                if (fusion_dir == '-'):
                    right_ending_ls = [fusion_endpoint]*len(end_jun_lk_ls)
                else:
                    right_ending_ls = [-1]*len(end_jun_lk_ls)
        ...
        for start_junlk_str in start_junlk_str_ls:
            ...
            for end_junlk_str in end_junlk_str_ls:
                ...
                if (len(item) > 0):
                    ...
                    # Special case for handling fusion single exons
                    #print line
                    if (left_ending_boundary != -1 or right_ending_boundary != -1):
                        if not candidate_dt[locus].has_key(item):
                            candidate_dt[locus][item] = {}
                        if not candidate_dt[locus][item].has_key(gpd_strand):
                            candidate_dt[locus][item][gpd_strand] = set()
                        ...
                        #     in merge_isoform_....py
                        if (left_ending_boundary == -1):
                            left_ending_boundary = right_ending_boundary - 200
                        if (right_ending_boundary == -1):
                            right_ending_boundary = left_ending_boundary + 200
                        twoending_str = str(left_ending_boundary) + '_' + str(right_ending_boundary)
                        candidate_dt[locus][item][gpd_strand].add( twoending_str )
                        if (fusion_dir != '*'):
                            print_str = 'kinfai'.join([locus, item, gpd_strand, twoending_str])
                            if (not fusion_isoform_2_readname_dict.has_key(print_str)):
                                fusion_isoform_2_readname_dict[print_str] = set()
                            fusion_isoform_2_readname_dict[print_str].add(ls[0])
                        #print(' '.join([str(item), str(left_ending_boundary), str(right_ending_boundary)]))
                                
                    if (left_ending_boundary == -1 and right_ending_boundary == -1):
                        print "Unexpected: both -1:" + line
                        exit(1)
...
###########################################################################################################
print fusion_isoform_2_readname_dict
    ...
    for item in candidate_dt[locus]:
        for gpd_strand in candidate_dt[locus][item]:
            for twoending_str in candidate_dt[locus][item][gpd_strand]:
                print_str = 'kinfai'.join([locus, item, gpd_strand, twoending_str])
                if (fusion_isoform_2_readname_dict.has_key(print_str)):
                    for readname in fusion_isoform_2_readname_dict[print_str]:
                        output.write( 'kinfai'.join([print_str, readname]) + '\n' )
                else:
