...
if len(sys.argv) >= 3:
    ...
    temp_foldername = sys.argv[3]
    #fusion_comp_margin = int(sys.argv[4])
    fusion_comp_margin = 0
else:
...
#    return -1 if incompatiable and gpd_ls is larger
    ...
    if ref_jun_str.find(jun_str) == -1:
        ...
        # If fusion end is not the same, they are compatible
        ref_fusion_candidate =  (ref_LR_gpd[0].split('|')[1])[0]
        tag_fusion_candidate =  (gpd_ls[0].split('|')[1])[0]
        if (ref_fusion_candidate != tag_fusion_candidate):
            # One of them is fusion candidate (not redundant)
            return 0
        if (tag_fusion_candidate == 'F'):   # Both are fusion-candidates
            ref_fusion_end = int(re.split(r'\+|\-', (ref_LR_gpd[0].split('|')[1]))[1])
            tag_fusion_end = int(re.split(r'\+|\-', (gpd_ls[0].split('|')[1]))[1])
            if (abs(ref_fusion_end - tag_fusion_end) > fusion_comp_margin):
                # They have different fusion junction end-point (keep both)
                return 0
...
# Get the list of super-read mapping for fusion candidates
redundant_reads_mapping_fusion = dict()
...
##            print Icompatiable
            ...
            if Icompatiable == 1:
                if ((gpd_ls[0].split('|')[1])[0] == 'F'):
                    redundant_reads_mapping_fusion[gpd_ls[0]] = ref_LR_gpd[0]
                break
            elif Icompatiable == -1:
                del_index_ls.append(j)
                if ((ref_LR_gpd[0].split('|')[1])[0] == 'F'):
                    redundant_reads_mapping_fusion[ref_LR_gpd[0]] = gpd_ls[0]
            j += 1    
#        print         del_index_ls
...
file = open(temp_foldername + "junfil_compatible_readnames_fusion.txt", 'w')
for i in redundant_reads_mapping_fusion.keys():
    file.write(i + '\t' + redundant_reads_mapping_fusion[i] + '\n')
file.close()
