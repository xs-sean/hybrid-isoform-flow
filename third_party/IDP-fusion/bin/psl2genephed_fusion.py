...
while (True):
    ...
    if (state == 0):
        ...
        if len(jun_start_ls) > 1:
            output_line1 = '\t'.join(result_ls)+'\t'+ ls[0] + '\t' + ls[10] + '\n'
        else:
            rejected_line1 = '\t'.join(result_ls)+'\t'+ ls[0] + '\t' + ls[10] + '\n'
        rejected_psl_line1 = line
    else:
        state = 0
        output_line2 = ''
        rejected_line2 = ''
        rejected_psl_line2 = ''
        if len(jun_start_ls) > 1:
            output_line2 = '\t'.join(result_ls)+'\t'+ ls[0] + '\t' + ls[10] + '\n'
        else:
            rejected_line2 = '\t'.join(result_ls)+'\t'+ ls[0] + '\t' + ls[10] + '\n'
        rejected_psl_line2 = line

        if ((rejected_line1 == '') and
            (rejected_line2 == '')):
            output.write(output_line1)
            output.write(output_line2)
        else:
            if (rejected_line1 == ''):
                rejected_line1 = output_line1
            if (rejected_line2 == ''):
                rejected_line2 = output_line2
            singleexon_output.write(rejected_line1)
            singleexon_psl_output.write(rejected_psl_line1)
            singleexon_output.write(rejected_line2)
            singleexon_psl_output.write(rejected_psl_line2)
    i+=1
output.close()
psl.close()
