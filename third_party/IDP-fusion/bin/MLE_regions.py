...
################################################################################
def fmax_coord_i(A, T, X, P, k, epsilon, step, gene):
    ...
    if it >= max_it:
        print('warning: maximum iteration exceeded (one-dim) for ' + gene)

    X[k] = x_old
    return [X, f, step_old]

################################################################################
def fmax_coord( A, T, X, P, n, epsilon, gene):
    [f, common_param_3] = log_likelihood(A, T, X, P, epsilon)
    it = 0
    max_it = 10000
    step = [1e3] * n 
    ...
    while (it < max_it):
        ...
        for i in range(n):
            [X, new_f, new_step] = fmax_coord_i(A, T, X, P, i, epsilon, step[i], gene)
    ...
    if it >= max_it:
        print('warning: maximum iteration exceeded for ' + gene)
    return [ X, f ]

################################################################################
def solve_likelihood( A, T , P, gene):
    epsilon = 1e-10
    [m, n] = A.shape
    X = asmatrix(ones(n)).T
    [X, f] = fmax_coord(A, T, X, P, n, epsilon, gene)
...
def iso_opt(A, L, N, NN, P, gene):
    ...
    [X_opt, f_max, I]= solve_likelihood(A, N, P, gene)
